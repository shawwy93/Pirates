...
public class LogbookMapDisplay : MonoBehaviour
{
    [Header("Entry")]
    [TextArea(6, 16)]
    [SerializeField] string entry =
        "<i>Ship's log, the forty-seventh day at sea.</i>\n\n" +
        "Three stacks on the horizon since the dawn watch. No canvas, " +
        "no rigging, and they keep pace with us without so much as a gust. " +
        "Coal-ships under a Crown flag. The old man won't turn. He says " +
        "we owe the new way nothing, and the wind still moves us for free.\n\n" +
        "I think tonight he's right. If the signal doesn't reach the fleet " +
        "by nightfall, they'll come through these waters blind.\n\n" +
        "<align=right><i>First mate's hand</i></align>";
    ...
    [SerializeField] Vector2 canvasSize        = new Vector2(0.30f, 0.24f);
    ...
    void BuildCanvas()
    {
        var existing = transform.Find("LogbookCanvas");
        if (existing != null)
        {
        ...
        var canvasGO = new GameObject("LogbookCanvas");
        if (textAnchor != null)
        {
        ...
        var textGO = new GameObject("LogbookText");
